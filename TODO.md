...
`kite-gateway`은 일반 Deployment로 실행한다. v1에서는 특정 노드 고정 없이
Kite namespace의 ServiceAccount 권한으로 CRD/Secret/Service를 읽는다.
...
install/dev 스크립트는 Linux systemd OpenSSH 환경에서 host OS의 기존
`sshd`가 22번을 쓰고 있으면 확인 후 2222로 옮길 수 있다.
...
- Service 생성 확인 추가
- gateway host key Secret 자동 생성
- Linux systemd OpenSSH 환경에서 host sshd 22번 handoff 확인과 적용
...
- gateway host key Secret 자동 생성
- Linux가 아니면 host sshd handoff 생략
- systemd OpenSSH가 없으면 host sshd handoff 생략
- host sshd가 이미 22번을 쓰지 않으면 변경하지 않음
- 필요한 경우 확인 후 `/etc/ssh/sshd_config`를 백업하고 host sshd를 2222로 이동
- `KITE_MANAGE_HOST_SSHD=true`이면 비대화식으로 host sshd handoff 수행
- `MANAGE_HOST_SSHD=false`이면 host sshd handoff 생략
...
- `/etc/kite/host-sshd`에 남긴 host sshd 백업과 state
...
clear/uninstall은 Kite가 만든 `/etc/kite/host-sshd/state.env`가 있을 때만
host sshd config를 백업본으로 복구한다. 상태 파일이 없으면 host sshd
복구는 no-op이다.
...
- [x] install/dev 스크립트에서 host sshd handoff 확인과 적용 추가
- [x] clear/uninstall 스크립트에서 host sshd 백업 복구 추가
