...
const accessLevelDocs: readonly AccessLevelDoc[] = [
  {
    ...
    title: 'No VM Create',
    tone: '권한 요청 단계',
    summary: '로그인은 가능하지만 VM 생성과 제어는 막는다.',
    rules: ['Dashboard에서 관리자 연락처를 안내한다.', 'API 직접 호출로 VM 생성/수정/전원/삭제를 시도해도 403을 반환한다.'],
  },
  {
    level: '1',
    title: 'Fixed Self-service',
    tone: '일반 사용자',
    summary: '자기 namespace 안에서 고정 스펙 VM만 다룬다.',
    rules: ['CPU 2, Memory 4Gi, Disk 25Gi로 고정한다.', '직접 생성 가능한 VM은 최대 2개로 제한한다.'],
  },
  {
    level: '2',
    title: 'Power User',
    tone: '팀 운영자',
    summary: '자기 VM은 자유 스펙으로 만들고, 사용자 권한 0/1만 조정한다.',
    rules: ['다른 유저 VM의 전원/삭제/설정 변경은 허용하지 않는다.', '관리 화면에서는 0과 1 사이의 권한 변경만 노출한다.'],
  },
  {
    level: '3',
    title: 'Full Admin',
    tone: '전체 관리자',
    summary: 'Kite의 모든 관리 작업을 수행할 수 있다.',
    rules: ['전체 VM 제어, 사용자 삭제, 시스템 설정, VM Offer 생성/삭제를 허용한다.', '운영 위험이 큰 작업은 UI에서 명확히 구분한다.'],
  },
];
...
const offerTimelineItems: TimelineProps['items'] = [
  {
    ...
    content: (
      <div>
        <Text strong>Admin이 Offer 생성</Text>
        <Paragraph className="docs-timeline-copy">대상 사용자 namespace에 CPU, memory, disk, image, 만료 시간을 가진 `KiteVirtualMachineOffer`를 만든다.</Paragraph>
      </div>
    ),
  },
  {
    ...
    content: (
      <div>
        <Text strong>User가 Claim</Text>
        <Paragraph className="docs-timeline-copy">유저는 자기 namespace의 Offer만 보고, VM name, domain prefix, SSH ID, 초기 비밀번호를 채운다.</Paragraph>
      </div>
    ),
  },
  {
    ...
    content: (
      <div>
        ...
        <Paragraph className="docs-timeline-copy">Claim 성공 시 Offer 스펙과 유저 입력을 합쳐 `KiteVirtualMachine`을 만들고 Offer는 정리한다.</Paragraph>
      </div>
    ),
  },
  {
    ...
    content: (
      <div>
        ...
        <Paragraph className="docs-timeline-copy">사용되지 않은 Offer는 controller cleanup으로 삭제해 더미 예약이 쌓이지 않게 한다.</Paragraph>
      </div>
    ),
  },
];
      ...
      <Content className="app-main app-main--wide docs-content">
        <section className="docs-hero">
          ...
          <Paragraph className="docs-subtitle">
            Kite의 권한 모델, VM 생성 제한, 관리자가 유저에게 VM 스펙을 배정하는 Offer 흐름을 한 곳에 정리합니다.
            지금 구현할 정책과 아직 운영상 주의해야 할 한계를 함께 둡니다.
          </Paragraph>
          <Space size={12} wrap>
        ...
        <section className="docs-section">
          <div className="docs-section-heading">
            ...
            <Text type="secondary">숫자가 높을수록 더 넓은 행동을 허용하지만, Level 2와 Level 3은 명확히 분리합니다.</Text>
          </div>
          <Row gutter={[16, 16]}>
          ...
          <Row gutter={[20, 20]} align="stretch">
            <Col xs={24} lg={8}>
              <Card className="docs-policy-card" hoverable>
                ...
                <Paragraph>
                  일반 사용자는 직접 VM을 만들 수 있지만 스펙은 고정합니다. UI 제한만 믿지 않고 backend에서도 같은 정책을 강제합니다.
                </Paragraph>
            ...
            <Col xs={24} lg={8}>
              <Card className="docs-policy-card" hoverable>
                ...
                <Paragraph>
                  Level 2는 자기 VM을 자유 스펙으로 만들 수 있지만 전체 관리자 권한은 아닙니다. 유저 권한은 0과 1 사이에서만 바꿉니다.
                </Paragraph>
                <Alert type="info" showIcon title="Level 2는 다른 유저 VM의 전원, 삭제, settings 변경을 하지 않는다." />
              </Card>
            </Col>
            <Col xs={24} lg={8}>
              <Card className="docs-policy-card" hoverable>
                <LockOutlined className="docs-card-icon" />
                <Title level={3}>Admin Contact</Title>
                <Paragraph>
                  Level 0 사용자는 VM 생성 버튼 대신 관리자 연락 안내를 봅니다. 연락처는 `kite-runtime-config`에서 관리합니다.
                </Paragraph>
                <Alert type="warning" showIcon title="adminContact는 email, Slack, 전화번호, URL 등 자유 텍스트로 시작합니다." />
              </Card>
            </Col>
          </Row>
        </section>
        ...
        <section className="docs-section docs-offer-section">
          <div className="docs-section-heading">
            ...
            <Text type="secondary">관리자가 스펙을 먼저 잡고, 유저가 나머지를 채워 자기 VM으로 claim하는 흐름입니다.</Text>
          </div>
          <Row gutter={[24, 24]}>
            <Col xs={24} lg={10}>
              <Card className="docs-offer-card" hoverable>
                ...
                <Paragraph>
                  ...
                  Offer를 별도 CRD로 분리하면 claim, 만료, 권한 감사가 더 단순해집니다.
                </Paragraph>
                <Divider />
                <Space size={8} wrap>
                  <Tag>Namespaced</Tag>
          ...
          <Card className="docs-limit-card">
            ...
            <Row gutter={[20, 12]}>
              ...
              <Col xs={24} md={8}>
                ...
                <Paragraph>Admin Offer는 명시적 배정이므로 Level 1의 고정 스펙 제한을 우회할 수 있습니다.</Paragraph>
              </Col>
              <Col xs={24} md={8}>
