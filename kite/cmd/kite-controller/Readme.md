...
- [x] `spec.delete = true`이면 KubeVirt VirtualMachine을 삭제한다.
- [x] `spec.delete = true`이고 KubeVirt VirtualMachine이 이미 없으면 KiteVirtualMachine CRD도 삭제한다.
- [ ] 실제 KubeVirt 상태를 읽어 `status.phase`에 반영한다.
- [ ] 실패 원인을 `status.conditions`에 남긴다.
- [x] KiteVirtualMachine이 삭제되면 같은 namespace/name의 KubeVirt VirtualMachine을 정리한다.
- [x] cleanup finalizer로 실제 KubeVirt VM 정리 전 KiteVirtualMachine CRD가 먼저 사라지지 않게 한다.

## gRPC 폐기

컨트롤러 gRPC 서버는 사용하지 않습니다. Kite의 기본 흐름은 `HTTP API -> Kite CRD -> controller reconcile`입니다.

API 서버는 Kubernetes API server에 `KiteUser`와 `KiteVirtualMachine` CRD를 기록합니다. Kubernetes API server는 이 CRD를 etcd에 저장합니다. 컨트롤러는 CRD와 KubeVirt 리소스를 watch하고, CRD spec을 원하는 상태로 보고 실제 Kubernetes/KubeVirt 리소스를 맞춥니다.

따라서 아래 방향으로 정리합니다.

- [ ] API 서버에서 컨트롤러 gRPC 서버로 요청을 보내지 않는다.
- [ ] 컨트롤러를 명령형 RPC 서버처럼 사용하지 않는다.
- [ ] protobuf Go 코드 생성은 하지 않는다.
- [ ] 남아 있는 gRPC 서버 초안과 proto 파일은 별도 정리 작업에서 삭제한다.
