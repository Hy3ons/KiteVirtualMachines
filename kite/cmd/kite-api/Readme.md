...
- `POST /api/v1/auth/signup`: 공개 가입. 현재 첫 가입자는 admin, 이후 가입자는 read-only로 생성
...
- `POST /api/v1/admin/runtime-secrets/rotate`: admin 권한으로 `kite/kite-runtime-secret`의 `jwtSecret`, `passwordSalt` 재생성
- `POST /api/v1/admin/cert`: admin 권한으로 `kite/global-tls-secret` TLS Secret 생성/수정
- JWT access token 발급과 검증
- Kubernetes dynamic client 연결
- `kite/kite-runtime-config` ConfigMap과 `kite/kite-runtime-secret` Secret 자동 생성

VM 생성 요청의 `sshPassword`는 HTTP request body에서만 평문으로 다룹니다.
`vm.Service.Create`는 즉시 `auth.HashPassword`와 runtime `passwordSalt`로
`KiteVirtualMachine.spec.sshPasswordHash`를 만들고, 별도로 Linux SHA-512
crypt 형식의 guest OS password hash를 `<vm-name>-guest-login` Secret에
저장합니다. CRD에는 `spec.sshPassword` 평문 필드를 저장하지 않습니다.
`kite-gateway`는 같은 salt로 `spec.sshPasswordHash`를 검증하고, VM 웹
콘솔은 cloud-init이 설정한 guest OS password hash를 사용합니다.

VM 생성 후 `sshPassword` 변경은 지원하지 않습니다. gateway SSH 진입점과
VM 내부 OS 콘솔 로그인은 서로 다른 경로이지만, 초기 생성 비밀번호에서
각각 필요한 hash를 파생하기 때문에 생성 후에는 Kite가 비밀번호를 복구하거나
변경할 수 없습니다.
...
`kite-api`는 `.env` 파일 없이 실행된다. 시작 시 Kubernetes dynamic client로
`kite` namespace, `kite/kite-runtime-config` ConfigMap,
`kite/kite-runtime-secret` Secret을 확인하고, 없으면 생성한다. 예전 버전에서
ConfigMap에 저장된 `jwtSecret`과 `passwordSalt`가 있으면 Secret으로 옮기고
ConfigMap에서는 제거한다.
...
현재 구현은 첫 번째 공개 signup 사용자를 admin으로 만든다. 이 방식은 작은
단일 운영자 클러스터에서는 단순하지만, 공개 도메인을 먼저 열어둔 상태에서는
의도하지 않은 사용자가 첫 admin이 될 수 있다. 그래서 production 운영에서는
외부 Ingress를 열기 전에 port-forward 또는 내부 네트워크에서 첫 admin을 먼저
만들고, 곧바로 base domain/TLS/HTTPS 설정을 마친 뒤 공개 접속을 열어야 한다.

장기적으로는 첫 admin 생성을 bootstrap token 또는 invite 기반으로 바꾸는 것이
권장된다. 그 변경 전까지는 첫 admin 생성 절차를 설치 runbook의 명시 단계로
취급한다.
...
- 내부 호출 흐름:
  ...
  7. 기존 사용자가 없으면 `AccessLevelAdmin(3)`, 한 명이라도 있으면 `AccessLevelReadOnly(0)`를 넣는다. production에서는 공개 Ingress를 열기 전에 운영자가 직접 이 첫 signup을 완료해야 한다.
  ...
  10. `spec.profile_image`는 호환성 필드로 남겨두되 빈 문자열로 저장한다.
...
- 내부 호출 흐름:
  ...
  5. `applyUpdate`가 email, password, namespace, access level 중 전달된 field만 반영하고 profile image 입력은 무시한다.
