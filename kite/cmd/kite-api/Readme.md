...
- `PATCH /api/v1/vms/:name/power`: 현재 사용자 namespace의 VM 희망 전원 상태 수정
...
### `GET /api/v1/me`, `GET /api/v1/admin/users`
...
- 내부 호출 흐름:
  1. admin 권한을 확인한다.
  2. 이후 흐름은 `POST /api/signup`과 같다.
  3. 현재 구현상 첫 사용자 여부에 따라 access level을 정한다. admin 페이지에서 특정 level을 직접 지정하는 생성 API가 필요하면 별도 request field와 service method를 추가해야 한다.
